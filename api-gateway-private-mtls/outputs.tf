...
output "alb_dns_name" {
  description = "DNS del ALB interno (punto de entrada mTLS)"
...
output "curl_example_direct_vpce" {
  description = "Ejemplo de curl directo al VPC Endpoint (sin pasar por ALB, sin mTLS)"
...
output "curl_example_direct_apigw" {
  description = "Ejemplo de curl directo al API Gateway (sin pasar por ALB, sin mTLS)"
...
output "ssm_download_certs_command" {
  ...
  value       = <<-EOT
    aws ssm send-command \
      ...
      --parameters 'commands=[
        "aws s3 cp s3://${aws_s3_bucket.truststore.bucket}/mtls/client.crt /etc/mtls/client.crt",
        "aws s3 cp s3://${aws_s3_bucket.truststore.bucket}/mtls/client.key /etc/mtls/client.key",
